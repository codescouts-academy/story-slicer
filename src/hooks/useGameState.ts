...
export function useGameState() {
  const [gameScreen, setGameScreen] = useState<'home' | 'tutorial' | 'level-select' | 'game' | 'results' | 'patterns'>('home');
  ...
  const updateStory = useCallback((id: string, updates: Partial<CreatedStory>) => {
  ...
  const validateAndSubmit = useCallback(() => {
    ...
    const validatedStories = createdStories.map(story => {
      const fullText = `Como ${story.role} quiero ${story.action} para ${story.benefit}`.toLowerCase();
      const isValid = level.expectedStories.some(expected => {
        const expectedFull = `Como ${expected.role} quiero ${expected.action} para ${expected.benefit}`.toLowerCase();
        // Fuzzy match - check if key words are present
        const keywords = expected.action.toLowerCase().split(' ').filter(w => w.length > 3);
      ...
      return {
        ...story,
        isValid,
        feedback: isValid ? '¡Excelente historia!' : 'Esta historia podría mejorarse',
      };
    });
    ...
    const patternBonus = correctCount >= level.expectedStories.length * 0.75 ? level.bonusPoints : 0;
    const noHintsBonus = hintsUsedCount === 0 ? 200 : 0;
    const speedBonus = timeSpent < 120 ? 100 : 0;
    const totalLevelScore = basePoints + patternBonus + noHintsBonus + speedBonus;
    ...
    if (!newAchievements.includes('no-hints') && hintsUsedCount === 0 && correctCount >= level.expectedStories.length * 0.5) {
      newAchievements.push('no-hints');
    }
    if (!newAchievements.includes('speed-demon') && timeSpent < 120 && correctCount >= level.expectedStories.length * 0.5) {
      newAchievements.push('speed-demon');
    }
    if (!newAchievements.includes('spidr-supreme') && level.id === 7 && stars >= 2) {
    ...
    const newProgress: GameProgress = {
      ...
      scores: {
        ...progress.scores,
        [currentLevelId]: Math.max(progress.scores[currentLevelId] || 0, totalLevelScore),
      },
      stars: {
        ...progress.stars,
